...
# funkcia ktora vybera styri bunky potrebne na interpolaciu z povodnych deviatich
def markActiveFields(result, normalizedX, normalizedY, lat, lon):
	...
	if y0 < y1: # pokial vzdialenost od lavej je vacsia ako od pravej tak ma lava strana nezaujima (neberiem do uvahy lave bunky)
		...
		result[0.5][-0.5]['active'] = False
		direction = 0.5
	if x0 > x1: # ci vzdialenost od lavej vrch bunky je vacsia ako vzdialenost od spodku bunky
...
# hodnota metriky sa vztahuje k stredu bunky (uvazuje sa tak pri bilinearnej interpolacii). Ja potrebujem normalizovat suradnice, pretoze stred bunky ma vzdy za desatinnou ciarkou hodnotu *.0.25 alebo *. 0.75
def normalize(number):
	normalized = round(number * 4) / 4 # normalizacia suradnice, aby bolo za desatinnou ciarkou suradnice bud 0.25 alebo 0.75
