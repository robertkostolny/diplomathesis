...
def parseFile(file_name, lat, lon):
			...
			if col['active']:
				for point in filtered:
					if hasattr(point, 'values') and point.values.any():
						...
						if point.parameterName == 'Surface pressure':
	...
	M = interpolate(selected[0][0], selected[0][1], station, 'lat', 'lon')	
	N = interpolate(selected[1][0], selected[1][1], station, 'lat', 'lon')
	X = interpolate(M, N, station, 'lon', 'lat')
